...
// Top-level view mode (4 tabs: Characters, Best Teams, Pull Advisor, Banner Advisor)
const viewMode = ref<'characters' | 'best-teams' | 'pull-advisor' | 'banner-advisor'>('characters');
...
useHead({
  title: computed(() => {
    ...
    if (viewMode.value === 'pull-advisor') {
      return 'HSR Pull Advisor - StarGuide | Honkai Star Rail';
    }
    if (viewMode.value === 'banner-advisor') {
      return 'HSR Banner Advisor - StarGuide | Honkai Star Rail';
    }
    if (selectedCharacter.value) {
      return `${selectedCharacter.value.name} HSR Teams - StarGuide | Honkai Star Rail`;
    }
...
// Handle browser back button for character deselection
const handlePopState = (event: PopStateEvent) => {
  // Only handle if on mobile and a character is currently selected
  if (isMobile.value && selectedCharacter.value) {
    // Check if we're going back from a character view
    const state = event.state;
    if (!state || !state.characterSelected) {
      // Going back to a state without a character selected
...
onMounted(async () => {
  ...
  // Browser history listener for mobile back button
...
watch(selectedCharacter, (newChar, oldChar) => {
  if (newChar) {
    ...
    // Only push if we're actually selecting a new character (not deselecting)
    if (isMobile.value) {
      window.history.pushState(
        { characterSelected: true, characterId: newChar.id },
        '',
        window.location.href
      );
    }
  } else if (oldChar && isMobile.value) {
    // Character was deselected - check if we should go back in history
    // Only go back if the current state has characterSelected flag
    const currentState = window.history.state;
    if (currentState && currentState.characterSelected) {
      // User clicked the back button, state was already handled by popstate
      // Do nothing
    }
  }

  // Investment tab is now shown for all characters, no need to switch away
});
...
const handleMobileBackButton = () => {
  if (window.history.state?.characterSelected) {
    window.history.back();
  } else {
    selectedCharacter.value = null;
  }
...
const handleCharacterSelect = (character: Character) => {
  if (selectedCharacter.value?.id === character.id) {
    // Deselecting - go back in history if on mobile
    if (isMobile.value && window.history.state?.characterSelected) {
      window.history.back();
    } else {
      selectedCharacter.value = null;
    }
      ...
      <main class="flex-1 p-4 md:p-6 lg:p-8 pb-20 md:pb-8 overflow-x-hidden">
        ...
        <KeepAlive>
          <PullAdvisorView
            ...
            :game-mode="gameMode"
            class="max-w-6xl mx-auto overflow-visible"
            @select-character="(id) => { selectedCharacter = characters.find(c => c.id === id) || null; viewMode = 'characters'; }"
          />
        </KeepAlive>
