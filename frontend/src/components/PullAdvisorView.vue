...
// Filter enriched supports by selected DPS (if any selected)
const filteredEnrichedSupportsForDPS = computed((): EnrichedRecommendation[] => {
  // If no DPS filter selected, show all
  if (selectedDPSIds.value.size === 0) {
    return enrichedSupportsForDPS.value;
  }

  // Filter to supports that are wanted by at least one selected DPS
  return enrichedSupportsForDPS.value.filter(rec => {
    // Check if any of the DPS that want this support are in our filter
    return rec.wantedBy.some(w => {
      const dpsChar = props.ownedCharacters.find(c => c.name === w.name);
      return dpsChar && selectedDPSIds.value.has(dpsChar.id);
    });
  });
});
