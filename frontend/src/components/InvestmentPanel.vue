          ...
          >
            <div class="flex items-center gap-4">
              ...
              <div class="flex-1 min-w-0">
                <span class="font-medium text-base">{{ lc.name }}</span>
    ...
    <div class="p-6 bg-gradient-to-br from-green-900/20 to-gray-900/20 rounded-lg border border-green-500/30">
      ...
      <div class="flex items-center justify-between">
        ...
        <div>
          ...
          <div class="text-xs text-gray-400 mt-1">tier pts from your eidolons</div>
        </div>
      ...
      <div class="space-y-3">
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <!-- Left: Icon + Description (vertical stack) -->
                ...
                <div v-if="eid.synergyBoosts.length > 0" class="mt-3 space-y-2">
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <span class="font-medium text-amber-300 text-sm">{{ boost.characterName }}</span>
                      <span
            ...
            <!-- Right: Tier Contribution Badge -->
            <div class="flex-shrink-0">
              <div class="text-right">
                <!-- OWNED: Celebratory green -->
                <template v-if="eid.isOwned">
                  <div class="text-green-400 text-sm font-semibold">
                    {{ eid.impactLabel }}
                  </div>
                  <div class="text-xs text-green-500 mt-1">
                    +{{ eid.impactValue.toFixed(1) }} tier pts
                  </div>
                </template>
                <!-- UNOWNED: Opportunity framing -->
                <template v-else>
                  <div :class="eid.impactColor" class="text-sm font-semibold">
                    {{ eid.impactLabel }}
                  </div>
                  <div class="text-xs text-gray-500 mt-1">
                    +{{ eid.impactValue.toFixed(1) }} available
                  </div>
                </template>
                <!-- Community stats -->
                <div v-if="getEidolonCommunityStats(eid.level)" class="community-stats-row mt-2">
                  <div
    ...
    <div class="p-6 bg-gradient-to-br from-purple-900/20 to-gray-900/20 rounded-lg border border-purple-500/30">
      ...
      <div class="flex items-center justify-between">
        ...
        <div>
          ...
          <div class="text-xs text-gray-400 mt-1">tier pts from your light cone</div>
        </div>
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-start justify-between gap-4">
              <div class="flex-1 flex gap-4">
                ...
                />
                <div class="flex-1 min-w-0">
                  <h4 class="font-semibold text-gray-100 text-lg">{{ lc.name }}</h4>
                  <div class="flex items-center gap-2 flex-wrap mt-1">
                    <span class="text-sm text-gray-400">{{ '★'.repeat(lc.rarity) }}</span>
                    <span
                      v-if="lc.isSignature"
              ...
              <!-- Tier Contribution Comparison -->
              <div class="text-right space-y-1">
                <div class="text-xs text-gray-400">
                  S1: <span class="text-purple-400 font-medium">+{{ lc.s1Power.toFixed(1) }} pts</span>
                </div>
                <div class="text-xs text-gray-400">
                  S5: <span class="font-medium" :class="lc.isSignature ? 'text-green-400' : 'text-purple-400'">+{{ lc.s5Power.toFixed(1) }} pts</span>
                  <span v-if="lc.isSignature" class="text-green-500 text-xs">(best)</span>
                </div>
                <!-- Community stats -->
                <div v-if="getLcCommunityStats(lc.id)" class="community-stats-row justify-end mt-1">
                  <div
