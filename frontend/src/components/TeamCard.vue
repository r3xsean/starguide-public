...
<template>
  ...
  >
    ...
    <div v-if="selected" class="selection-indicator">
      ...
      <div class="indicator-glow" :style="{ background: `radial-gradient(ellipse at center, ${focalElementColor}30, transparent 70%)` }"></div>
    </div>
    ...
    <div class="card-content">
      ...
      <template v-if="compact">
        <div class="compact-layout">
          ...
          <!-- Arrow -->
          <div class="compact-arrow" :class="{ visible: selected }">›</div>
        </div>
      </template>
...
.team-card.selected {
  border-color: var(--color-stellar-500, #f99307);
  box-shadow:
    0 0 0 2px rgba(249, 147, 7, 0.25),
...
.team-card.selected .bg-gradient {
  opacity: 1;
}
...
.team-card.selected .bottom-accent {
  opacity: 1;
}
...
.compact-arrow.visible,
.team-card.compact:hover .compact-arrow {
  opacity: 1;
  transform: translateX(0);
}

.team-card.compact.selected .compact-arrow {
  color: var(--color-stellar-500, #f99307);
}
