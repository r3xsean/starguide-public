...
const teamDetailPanelRef = ref<HTMLElement | null>(null);
...
// Game mode to pass to CharacterCard (undefined for 'avg' to use averaged tier)
const characterCardGameMode = computed((): GameMode | undefined => {
  return selectedMode.value === 'avg' ? undefined : selectedMode.value;
});

// Sync with parent gameMode prop (only affects effective mode, not display selection)
watch(() => props.gameMode, () => {
  // When parent mode changes, clear selection since teams may re-sort
  selectedTeamIndex.value = null;
});

// Clear selection when mode changes (teams will re-sort)
watch(selectedMode, () => {
  selectedTeamIndex.value = null;
});

// Character filter functions
const toggleCharacterFilter = (characterId: string) => {
  if (selectedCharacterIds.value.has(characterId)) {
    selectedCharacterIds.value.delete(characterId);
  } else {
    selectedCharacterIds.value.add(characterId);
  }
  selectedCharacterIds.value = new Set(selectedCharacterIds.value); // Trigger reactivity
  selectedTeamIndex.value = null; // Clear team selection when filter changes
};

const clearCharacterFilters = () => {
  selectedCharacterIds.value.clear();
  selectedCharacterIds.value = new Set(selectedCharacterIds.value); // Trigger reactivity
  selectedTeamIndex.value = null;
};
...
const handleCharacterSelect = (character: Character) => {
  toggleCharacterFilter(character.id);
};
...
// Handle team selection with scroll (toggle behavior)
...
const selectedTeam = computed(() => {
  ...
  return sortedTeams.value[selectedTeamIndex.value] || null;
});
</script>
...
<template>
  <div class="best-teams-view">
    ...
    <div v-else class="teams-content">
      ...
      <!-- Character Filter -->
      <div v-if="ownedCharacters.length > 0" class="character-filter-section">
        <div class="filter-header">
          <button class="add-character-btn" @click="openCharacterModal">
            <span class="btn-icon">+</span>
            <span class="btn-text">Add Character</span>
          </button>
          <button v-if="selectedCharacterIds.size > 0" class="clear-filters-btn" @click="clearCharacterFilters">
            Clear All ({{ selectedCharacterIds.size }})
          </button>
        </div>
        ...
        <div v-if="selectedCharacters.length > 0" class="selected-characters-grid">
          <CharacterCard
            ...
            :show-tier="false"
            :show-name="true"
            :game-mode="characterCardGameMode"
            size="md"
            @click="toggleCharacterFilter(char.id)"
          />
        </div>
      </div>
        ...
        <div v-if="showCharacterModal" class="modal-overlay" @click="closeCharacterModal">
          <div class="modal-content" @click.stop>
            <div class="modal-header">
              <h2 class="modal-title">Select Characters</h2>
            ...
            <div class="modal-body">
              <CharacterGrid
                ...
                :roster="new Map(ownedCharacters.map(c => [c.id, selectedCharacterIds.has(c.id) ? 'owned' : 'none']))"
                :game-mode="characterCardGameMode"
                @select="handleCharacterSelect"
              />
            </div>
            <div class="modal-footer">
              <div class="selected-count">{{ selectedCharacterIds.size }} character{{ selectedCharacterIds.size !== 1 ? 's' : '' }} selected</div>
              <button class="modal-done-btn" @click="closeCharacterModal">Done</button>
            </div>
          </div>
        </div>
      </Transition>
        ...
        <Transition name="detail-slide">
          <div v-if="selectedTeam" ref="teamDetailPanelRef" class="teams-detail-panel">
            <TeamDetail
              ...
              :get-ownership="getOwnership"
              :get-investment="getInvestment"
              :focal-character-id="selectedTeam.characters[0]?.id"
              :game-mode="effectiveGameMode"
            />
          </div>
        </Transition>
...
.add-character-btn:hover {
  background: linear-gradient(135deg, rgba(99, 102, 241, 0.4) 0%, rgba(99, 102, 241, 0.25) 100%);
...
.modal-content {
  background: linear-gradient(135deg, rgba(18, 18, 38, 0.98) 0%, rgba(12, 12, 28, 0.98) 100%);
  border: 1px solid rgba(255, 255, 255, 0.1);
  border-radius: 1rem;
  max-width: 900px;
  width: 100%;
...
.modal-done-btn {
  ...
  font-weight: 600;
  color: white;
  background: linear-gradient(135deg, rgba(99, 102, 241, 0.6) 0%, rgba(99, 102, 241, 0.5) 100%);
...
.modal-done-btn:hover {
  background: linear-gradient(135deg, rgba(99, 102, 241, 0.7) 0%, rgba(99, 102, 241, 0.6) 100%);
...
@media (max-width: 768px) {
  .modal-overlay {
    padding: 1rem;
    /* Add bottom padding for mobile tab bar (64px) + safe area */
    padding-bottom: calc(64px + env(safe-area-inset-bottom, 0px) + 1rem);
  }

  .modal-content {
    /* Reduce max-height to account for tab bar */
    max-height: calc(100vh - 64px - env(safe-area-inset-bottom, 0px) - 2rem);
  }
