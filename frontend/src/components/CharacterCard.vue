...
// Character icon URL based on character ID
const iconUrl = computed(() => {
  return `/icons/${props.character.id}.webp`;
});
...
<template>
  ...
  >
    ...
    <div
      v-if="eidolon"
      class="eidolon-badge"
      :class="isXs && 'eidolon-badge-xs'"
      :title="`Eidolon ${eidolon}`"
    >
      E{{ eidolon }}
    </div>
...
.ownership-glow-concept {
  ...
  border: 2.5px solid rgba(217, 70, 239, 0.8);
}
...
/* Eidolon Badge (top-right) - Enhanced visibility */
.eidolon-badge {
  position: absolute;
  top: -4px;
  right: -4px;
